...
const OrederdReportForAllUser = () => {
      ...
      useEffect(()=>{
        const fetchProduct=async()=>{
          const res=await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/report/get-all-placed-order-withou-user-report`, {withCredentials: true})
  ...
  return (
     <div className="overflow-x-auto w-full">
       <div className='flex justify-between mb-3 mt-4'>
        <h4>Placed Order Report Of All Users</h4>
      ...
      <table className="min-w-full border border-gray-300">
        <thead>
          <tr className="bg-gray-100 text-left">
            ...
            <th className="px-3 py-2 border">Razorpay Order</th>
            ...
            order.items?.map((item, index) => {
              ...
              return (
                <tr key={item._id} className="hover:bg-gray-50">
                  ...
                  {/* Show customer name only for first row of each order */}
                  <td className="px-3 py-2 border font-medium">
                    {index === 0 ? order?.address?.fullName : ""}
                  </td>
                  ...
                  {/* Razorpay Order ID only on first row */}
                  <td className="px-3 py-2 border">
                    {index === 0 ? order?.razorpayOrderId || "-" : ""}
                  </td>
                  ...
                  {/* Show order total only on first row */}
